...
impl GlueCatalogProvider {
    ...
    async fn register_glue_table(
        ...
        let listing_options: ListingOptions =
            Self::get_listing_options(database_name, table_name, glue_table, &sd)?;
    ...
    fn get_table_name(glue_table: &Table) -> &str {
    ...
    fn calculate_options(glue_table: &Table, sd: &StorageDescriptor) -> Result<ListingOptions> {
        ...
        let partition_col_names = glue_table
            .partition_keys()
            .iter()
            .map(|c| (c.name().to_owned(), DataType::Utf8))
            .collect::<Vec<(String, DataType)>>();

        let file_extension = format.file_type().get_ext().clone();

        let listing_options = ListingOptions {
            file_extension,
            format: Arc::from(format),
            table_partition_cols: partition_col_names,
            collect_stat: true,
            target_partitions: 1,
            file_sort_order: vec![],
        };
